...
- **Etiqueta automática de sector/región** (nivel 1, decisión de Pablo, 26-sep-2026): para cada ETF/fondo de bolsa del Núcleo con ISIN y sin `sub`, la etiqueta sale de la **composición real de EODHD** (`src/lib/mi-cartera/composicion.ts`, `/api/cartera/composicion?isin=`): una casilla domina si llega al **60 %** (sectores Morningstar agrupados en las siete casillas; regiones agrupadas en EE. UU. / Europa / Asia / Emergentes; Small + Micro ≥ 60 % → small caps); si ninguna región domina → **global**; si ningún sector domina, la estrategia sectorial no decide y la mixta cae a la región. `subPct` guarda el % con que se etiquetó (solo informativo). Siempre corregible a mano (lo manual no se toca); la sugerencia por nombre queda de respaldo cuando no hay composición. Caché: Upstash `composicion:v1:<ISIN>` 30 días (un null, 1 día) y localStorage `cartera-k:composicion:v1`. **El semáforo no cambia**: sigue siendo un activo en una casilla.
