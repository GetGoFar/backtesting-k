...
function buildTooltips(granularity: DisplayGranularity) {
  ...
  return {
    ...
    skewness:
      `Asimetría de la distribución de retornos ${plural === "meses" ? "mensuales" : plural === "días" ? "diarios" : "trimestrales"}. 0 = simétrica como una distribución normal. Negativa = cola izquierda más larga (pérdidas extremas más frecuentes que ganancias extremas — típico en banca privada y fondos de seguros que venden cisnes negros). Positiva = cola derecha más larga (loterías, momentum). Es una medida ESTANDARIZADA (relativa a la propia volatilidad de cada cartera), describe la FORMA de la distribución, no la magnitud de las pérdidas — eso lo miden VaR/CVaR/Max DD. Por eso una cartera puede tener mejor Sharpe y menor drawdown y aun así peor asimetría: si suele ser muy tranquila pero tiene un desplome puntual grande, ese desplome destaca más respecto a su propia distribución.`,
    excessKurtosis:
      "Curtosis en exceso = kurtosis - 3. Mide la probabilidad de eventos extremos vs una distribución normal. 0 = normal. >0 = colas gordas (cisnes negros más probables de lo que sugiere la volatilidad). Una cartera con baja vol pero alta kurtosis ESCONDE riesgo de cola. Es una medida ESTANDARIZADA (relativa a la propia volatilidad), describe la FORMA de la distribución, no la magnitud absoluta de las pérdidas (eso lo ven VaR/CVaR/Max DD). Por eso una cartera más tranquila de media pero con algún desplome puntual grande puede tener MÁS curtosis que un índice más volátil cuyo peor mes 'destaca menos'.",
    varHistorical:
