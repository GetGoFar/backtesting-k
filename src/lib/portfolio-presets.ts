...
const K_SECTORIAL_USA_PRESETS: PortfolioPreset[] = [
  ...
  },
  {
    ...
    description: "Riesgo 6: 20% VFITX + 20% Oro + 60% sectores (XLV/XLK 15% + XLP/XLE/VGSIX/XLU 7,5%)",
    type: "index",
    holdings: [
      { fundId: "spdr-xlp", weight: 7.50 },
      { fundId: "spdr-xlv", weight: 15.00 },
      { fundId: "spdr-xlk", weight: 15.00 },
...
// viven en portfolio-presets-privados.ts (solo servidor). El navegador de Pablo las recibe de
// /api/presets/privados cuando entra con su código personal (presets-privados-client.ts) y las registra
// con registrarPresetsPrivados(); getAllPresets() y getPresetById() las incluyen a partir de entonces.
let PRIVADOS: PortfolioPreset[] = [];
export function registrarPresetsPrivados(lista: PortfolioPreset[]): void {
...
export function getAllPresets(): PortfolioPreset[] {
  return PRIVADOS.length ? [...ALL_PRESETS, ...PRIVADOS] : ALL_PRESETS;
}
...
export function getPresetById(id: string): PortfolioPreset | undefined {
  return PRESETS_BY_ID.get(id) ?? PRIVADOS.find((p) => p.id === id);
}
...
export function getPresetsByType(type: FundType): PortfolioPreset[] {
  return getAllPresets().filter((preset) => preset.type === type);
}
